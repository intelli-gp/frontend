...
export const getEditTask = (task: Task, existingTask: Task): Partial<Task> => {
    ...
    const existingDueDateTime = moment(existingTask.DueDate).format().slice(0, 16);
    const existingStartDateTime = moment(existingTask.StartDate).format().slice(0, 16);

    if (task.DueDate!== existingDueDateTime || task.StartDate !== existingStartDateTime) {
